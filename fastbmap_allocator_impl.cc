...
inline interval_t _align2units(uint64_t offset, uint64_t len, uint64_t min_length)
{
  interval_t res;
  if (len >= min_length) {
    res.offset = round_up_to(offset, min_length);
    auto delta_off = res.offset - offset;
    if (len > delta_off) {
      res.length = len - delta_off;
      
      delta_off = res.length % min_length;
      res.length -= delta_off;
  ...
  for (auto pos = pos_start / d; pos < pos_end / d; ++pos) {
	...
	interval_t longest;
	    ...
	    (ctx->min_affordable_len == 0 ||
          ...
          auto delta = longest.length % min_length;
          ctx->min_affordable_len = longest.length - delta;
  ...
  int64_t l0_pos_end)
{
  ...
  int64_t pos_e = std::min(l0_pos_end, round_val);
  while (pos < pos_e) {
    (*val_s) &= ~bits;
    bits <<= 1;
    pos++;
